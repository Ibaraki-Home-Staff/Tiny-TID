...
1. `bunx wrangler d1 create tiny-tid` / `bunx wrangler kv namespace create SNAPSHOTS` → `wrangler.jsonc` のIDを更新
2. `bunx wrangler d1 migrations apply tiny-tid --remote`
3. `bunx wrangler secret put VAPID_PRIVATE_KEY` / `VAPID_SUBJECT`(鍵は `bunx web-push generate-vapid-keys`)
